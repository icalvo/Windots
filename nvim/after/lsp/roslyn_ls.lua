...
local function on_init_project(client, project_files)
  ...
  client:notify('project/open', {
    projects = vim.tbl_map(
      function(file) return vim.uri_from_fname(file) end,
      project_files
    ),
  })
end
  ...
  root_dir = function(bufnr, cb)
    ...
    if not bufname:match('^' .. fs.joinpath('/tmp/MetadataAsSource/')) then
      ...
      local root_dir = fs.root(
        bufnr,
        function(fname, _) return fname:match('%.sln[x]?$') ~= nil end
      )
      ...
      if not root_dir then
        ...
        root_dir = fs.root(
          bufnr,
          function(fname, _) return fname:match('%.csproj$') ~= nil end
        )
      end
